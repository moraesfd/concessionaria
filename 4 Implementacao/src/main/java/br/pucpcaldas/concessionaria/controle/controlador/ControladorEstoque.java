...
import br.pucpcaldas.concessionaria.controle.repositorio.OrganizacaoDeEstoqueEmMemoria;
import br.pucpcaldas.concessionaria.dominio.Estoque;

public class ControladorEstoque {
	
	private OrganizacaoDeEstoque organizacaoEstoque;
	private OrganizacaoDeEstoqueEmMemoria estoqueEmMemoria;
...
//	Método responsável por criar e inserir uma nova instância de Estoque em memória
	public boolean cadastraEstoqueProdutoEmMemoria(int idProduto, int quantidade){
		boolean retorno = false;
		if(idProduto == 0 || quantidade == 0){
			retorno = false;
		}else{
			Estoque estoque = new Estoque(idProduto, quantidade);
			try {
				estoqueEmMemoria = new OrganizacaoDeEstoqueEmMemoria();
				estoqueEmMemoria.cadastraEstoqueProduto(estoque);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				retorno = false;
			}
...
//	Método responsável por alterar informações sobre o Estoque em memória
	public boolean atualizaEstoqueEmMemoria(Estoque estoque){
		try {
			if(estoque.getQuantidade() < 0.0){
				return false;
			}else{
				estoqueEmMemoria = new OrganizacaoDeEstoqueEmMemoria();
				estoqueEmMemoria.atualizaEstoque(estoque);
				return true;
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
